...
public class JsonObjectMatchers {
  ...
  public static Matcher<List<JsonObject>> hasSoleValidationError(String message, String property) {
      ...
      protected boolean matchesSafely(List<JsonObject> errors) {
        return errors.stream().anyMatch(error -> {
          List<JsonObject> parameters = JsonArrayHelper.toList(
            error.getJsonArray("parameters"));

          return error.getString("message").equals(message) &&
